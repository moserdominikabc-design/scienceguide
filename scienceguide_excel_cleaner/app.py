...
class ColumnSelector(tk.Toplevel):
    def __init__(
        ...
        ttk.Label(
            outer,
            text="Factory defaults are preselected. Add, remove or reorder columns for this export.",
        ...
        ttk.Label(body, text="Included (output order)").grid(row=0, column=0, sticky="w")
        ttk.Label(body, text="Available from source").grid(row=0, column=2, sticky="w")
        ...
        ttk.Button(bottom, text="Factory defaults", command=self.reset_factory).pack(side="left")
        ttk.Button(bottom, text="Select all", command=self.select_all).pack(side="left", padx=(8, 0))
    ...
    def refresh(self):
        ...
        if missing:
            pieces.append(f"{len(missing)} factory field(s) are not present in this source")
        self.note.configure(text=" · ".join(pieces) if pieces else "All factory fields are available.")
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.status = tk.StringVar(value="Choose the VER and ANG exports to begin.")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            main,
            text="Select the two exports, adjust columns if needed, then create the cleaned Excel files.",
        ...
        cols = ttk.LabelFrame(main, text="3. Columns", padding=12)
    ...
    def choose_file(self, kind: str):
        ...
        try:
            ...
            selected, missing, aliases = factory_selection(kind, columns)
        ...
        self._update_column_status(kind, missing, aliases)
    ...
    def _update_column_status(self, kind: str, missing=None, aliases=None):
        ...
        text = f"{len(self.selected[kind])} of {len(self.columns[kind])} selected"
        notes = []
        if aliases:
    ...
    def process(self):
        for kind in ("VER", "ANG"):
            if not self.paths[kind].get():
                messagebox.showerror(APP_TITLE, f"Choose the {kind} file first.")
                return
        ...
        try:
            ...
            ver_output = folder / f"{export_date}_ver_mod.xlsx"
            ang_output = folder / f"{export_date}_ang_mod.xlsx"
            ver_result = process_workbook(self.paths["VER"].get(), self.selected["VER"], ver_output)
            ang_result = process_workbook(self.paths["ANG"].get(), self.selected["ANG"], ang_output)
        except Exception as exc:
        ...
        self.process_button.state(["!disabled"])
        self.status.set(
            f"Finished. VER: {ver_result['output_rows']} rows / {ver_result['output_columns']} columns. "
            f"ANG: {ang_result['output_rows']} rows / {ang_result['output_columns']} columns. "
            f"Saved to {folder}."
        )
        messagebox.showinfo(
            APP_TITLE,
